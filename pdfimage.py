...
pages = convert_from_path('perform.pdf', 500)

# loop through each page image and perform OCR
for i, page in enumerate(pages):
    # save page image as JPEG
    page.save(f'page_{i}.jpeg', 'JPEG')

    # read image file
    img = cv2.imread(f"page_{i}.jpeg")
    ...
    with open(f"page_recognized.txt", "a+") as file:
    ...
    cv2.imshow(f"Page {i+1} Contours", img_copy)
    cv2.waitKey(0)
